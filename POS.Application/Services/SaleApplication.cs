...
namespace POS.Application.Services
{
    public class SaleApplication : ISaleApplication
    {
        ...
        public async Task<BaseResponse<SaleDetailPdf>> ListSalesExportPdf(int saleId)
        {
            var response = new BaseResponse<SaleDetailPdf>();

            try
            {
                var sale = await _unitOfWork.Sale.GetAllQueryable()
                    .AsNoTracking()
                    .Include(x => x.VoucherDocumentType)
                    .Include(x => x.Client)
                    .Include(x => x.Warehouse)
                ...
                var saleDetails = await _unitOfWork.SaleDetail.GetSaleDetailBySaleId(sale.Id);
                sale.SaleDetails = saleDetails.ToList();

                response.IsSuccess = true;
                response.Data = _mapper.Map<SaleDetailPdf>(sale);
                response.Message = ReplyMessage.MESSAGE_QUERY;
            }
            catch (Exception ex)
            {
